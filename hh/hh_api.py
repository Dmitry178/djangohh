...
def get_vac(id_vac):
    ...
    try:
        ...
        if response['salary']:
            vac.append(
                '<b>Зарплата:</b>' + (
                    (' от ' + str(response['salary']['from'])) if response['salary']['from'] else '') + \
        ...
        if 'languages' in response:
            if response['languages']:
                ...
                vac.append('<br>'.join(vac1))
        vac_url = response['alternate_url']
...
def get_skills(query: str, region: int) -> dict:
    per_page = 100  # количество вакансий, в которых будут сканироваться ключевые навыки
    params = {
        'text': query,
        'per_page': per_page,
        'page': 0
